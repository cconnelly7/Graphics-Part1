void keyPressed()  // executed each time a key is pressed: sets the Boolean "keyPressed" till it is released   
    ...
    if(key=='5') {pointsCount=4;};
    if(key=='6') {pointsCount=6;};
    if(key=='7') ; 
    if(key=='8') ;
    if(key=='9') ;
    if(key=='0') method=0;
    ...
    if(key=='c') ; 
    if(key=='d') showBrezierEdges=!showBrezierEdges;  
    if(key=='e') drawEdges=!drawEdges;
    if(key=='f') fill=!fill;
    if(key=='g') ; 
    if(key=='h') ;
    if(key=='i') ;
    ...
    if (key == CODED) 
       ...
       if (keyCode == LEFT) { if (knotScalar > 0) {knotScalar-=0.05;} pressed="LEFT";   };
       if (keyCode == RIGHT) {if (knotScalar < 1){knotScalar+=0.05;} pressed="RIGHT";   };
